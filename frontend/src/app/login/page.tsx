...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      // Ambil token yang baru di-set di store untuk disimpan di Cookie
      const token = useAuthStore.getState().token;
      
      if (token) {
        // Simpan token ke cookie agar Middleware bisa membacanya
      ...
      const friendly = typeof raw === 'string' ? raw : 'Email atau password salah';
  ...
  const handleGoogleLogin = () => {
    ...
    window.location.href = `${API_BASE}/auth/google?redirect=/feed&mode=login`;
  };
