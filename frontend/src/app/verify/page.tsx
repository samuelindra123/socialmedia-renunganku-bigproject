...
function VerifyContent() {
  ...
  const verifyToken = async (tokenStr: string) => {
    setStatus('verifying');
    try {
      const { data } = await apiClient.get(`/auth/verify-email?token=${tokenStr}`);
      setStatus('success');
      ...
      setStatus('error');
      setErrorMessage(error.response?.data?.message || "Link verifikasi tidak valid atau sudah kadaluarsa");
    }
  };
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data } = await apiClient.post(`/auth/verify-otp/${userId}`, { 
        otp 
      });
