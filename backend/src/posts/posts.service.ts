...
// CDN URL for serving media (faster & saves bandwidth)
const CDN_URL = 'https://renunganku.sgp1.cdn.digitaloceanspaces.com';
const ORIGIN_URL = 'https://renunganku.sgp1.digitaloceanspaces.com';
...
export class PostsService {
  ...
  async getPostById(postId: string) {
    ...
    return {
      ...post,
      hashtags: tags,
      images: post.images.map((img) => ({ ...img, url: toCdnUrl(img.url) })),
      videos: post.videos.map((vid) => ({
        ...vid,
        url: toCdnUrl(vid.url),
        thumbnail: vid.thumbnail ? toCdnUrl(vid.thumbnail) : null,
